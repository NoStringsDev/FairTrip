...
| `wrangler deploy` / D1 / R2 | Prefer **not** running deploy from Cloud Agents unless you intend it; use `CLOUDFLARE_API_TOKEN` only in Cursor Cloud secrets if required, with minimal token scope. |
